...
# === 路径 1：FedProx ===
path1 = "exp/FedAvg_resnet18_on_FashionMNIST@torchvision_lr0.01_lstep1/sub_exp_20250617223112/eval_results.log"
rounds1, val_loss1 = parse_log_file(path1)

# === 路径 2：FedAvg ===
path2 = "/root/autodl-tmp/FederatedScope/exp/FedAvg_resnet18_on_FashionMNIST@torchvision_lr0.01_lstep1/sub_exp_20250611153704/eval_results.log"
rounds2, val_loss2 = parse_log_file(path2)


# === 绘图 ===
plt.figure(figsize=(30, 20))
plt.plot(rounds1, val_loss1, marker='o', linestyle='-', label='dp', color='b')
plt.plot(rounds2, val_loss2, marker='s', linestyle='--', label='nodp', color='r')
plt.xticks(rounds1[::5])
plt.xlabel("Round", fontsize=20)
plt.ylabel("test_acc", fontsize=20)
plt.title("Comparison of test_acc", fontsize=24)
plt.legend(fontsize=18)
plt.grid(True, linestyle='--', alpha=0.7)

# 设置纵坐标更精细
min_loss = min(min(val_loss1), min(val_loss2))
max_loss = max(max(val_loss1), max(val_loss2))
plt.ylim(min_loss - 0.1, max_loss + 0.1)
plt.yticks(np.linspace(min_loss - 0.1, max_loss + 0.1, num=20))
plt.gca().yaxis.set_major_formatter(ticker.FormatStrFormatter('%.2f'))

plt.tight_layout()
plt.savefig("dp_fashionemnist_test_acc.png", dpi=300, bbox_inches='tight')
