...
def load_caltech_dataset(config, client_cfgs=None):
    """
    Load Caltech dataset for federated learning based on RethinkFL implementation
    ...
    try:
        ...
        logger.info(f"Loaded Caltech dataset: train={len(train_dataset)}, test={len(test_dataset)}")

    except Exception as e:
        logger.error(f"Failed to load Caltech dataset: {e}")
        raise e
    ...
    logger.info(f"Successfully created federated Caltech dataset with {len(federated_data)} clients")
...
def call_caltech_data(config, client_cfgs=None):
    ...
    """
    if config.data.type.lower() == "caltech":
        logger.info("Loading Caltech dataset...")
        return load_caltech_dataset(config, client_cfgs)
    return None


# Register the Caltech dataset
register_data("caltech", call_caltech_data)