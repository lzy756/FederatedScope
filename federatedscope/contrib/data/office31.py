...
class Office31Dataset(Dataset):
    """Office-31 数据集读取器，支持三个域，每个域下按类组织。
...
def load_office31_data(config, client_cfgs=None):
    ...
    # 加载完整数据集
    root = "/home/lzy/Domainnet/Office-31"
    full_dataset = Office31Dataset(root)
...
def call_office31_data(config, client_cfgs=None):
    if config.data.type == "Office31":
        return load_office31_data(config, client_cfgs=client_cfgs)


register_data("Office31", call_office31_data)
