...
def sample_dataset(dataset, fraction):
    ...
    logger.info(
        f"从 {total_size} 个样本中采样 {sample_size} 个样本 (比例: {fraction:.3f})")
...
def load_usps(data_root, transform):
    ...
    X_tr_sp, y_tr = load_svmlight_file(os.path.join(usps_dir, 'usps'),
                                       n_features=256)
    X_te_sp, y_te = load_svmlight_file(os.path.join(usps_dir, 'usps.t'),
                                       n_features=256)
    ...
    return USPSDataset(X_tr, y_tr,
                       transform), USPSDataset(X_te, y_te, transform)
...
def load_syn(data_root, transform):
    ...
    train_ds = ImageFolder(root=os.path.join(syn_root, 'imgs_train'),
                           transform=transform)
    test_ds = ImageFolder(root=os.path.join(syn_root, 'imgs_valid'),
                          transform=transform)
...
class MNISTM(VisionDataset):
    ...
    def __init__(self,
        ...
        """
        super(MNISTM, self).__init__(root,
                                     transform=transform,
        ...
        logger.info(
            f"Loaded MNIST-M {'train' if train else 'test'} data: "
...
def _merge_data_dictionaries(config, data_dicts):
    ...
    total_clients = sum(len(data_dict) - 1
                        for data_dict in data_dicts)  # -1 to exclude server
    ...
    server_train = ConcatDataset(
        [data_dict[0].train_data for data_dict in data_dicts])
    server_val = ConcatDataset(
        [data_dict[0].val_data for data_dict in data_dicts])
    server_test = ConcatDataset(
        [data_dict[0].test_data for data_dict in data_dicts])
    merged_data_dict[0] = ClientData(tmp_config_merged,
                                     train=server_train,
                                     val=server_val,
                                     test=server_test)
...
def _perform_clustering_analysis(data_dict, data_root):
    ...
    try:
        ...
        for client_id in range(1, len(data_dict)):
            ...
            if hasattr(client_data,
                       'train_data') and client_data.train_data is not None:
                train_subsets.append(client_data.train_data)
            if hasattr(client_data,
                       'test_data') and client_data.test_data is not None:
        ...
        if train_subsets and test_subsets:
            ...
            train_distributions = get_label_distribution(train_subsets,
                                                         num_classes=10)
            test_distributions = get_label_distribution(test_subsets,
                                                        num_classes=10)
...
def calculate_client_allocation(dataset_sizes,
                                total_clients=100,
                                min_clients_per_domain=1):
    """
    根据数据集大小比例分配客户端数量
    ...
    for size in dataset_sizes:
        ...
        allocated = max(min_clients_per_domain,
                        int(proportion * total_clients))
    ...
    if current_total != total_clients:
        ...
        largest_domains = sorted(range(len(initial_allocation)),
                                 key=lambda i: initial_allocation[i],
                                 reverse=True)
...
def load_digits_data(config, client_cfgs=None):
    ...
    logger.info(
        f"Original MNISTM dataset: train={len(mnist_train)}, test={len(mnist_test)}"
    )
    mnist_train = sample_dataset(mnist_train, 1 / 6)
    mnist_test = sample_dataset(mnist_test, 1 / 6)
    logger.info(
        f"Sampled MNIST-M dataset: train={len(mnist_train)}, test={len(mnist_test)}"
    )

    # USPS数据集
    usps_train, usps_test = load_usps(data_root, transform)
    logger.info(
        f"Loaded USPS dataset: train={len(usps_train)}, test={len(usps_test)}")

    # SVHN数据集
    svhn_train, svhn_test = load_svhn(data_root, transform)
    logger.info(
        f"Original SVHN dataset: train={len(svhn_train)}, test={len(svhn_test)}"
    )
    svhn_train = sample_dataset(svhn_train, 1 / 7)
    svhn_test = sample_dataset(svhn_test, 1 / 7)
    logger.info(
        f"Sampled SVHN dataset: train={len(svhn_train)}, test={len(svhn_test)}"
    )

    # SYN数据集
    syn_train, syn_test = load_syn(data_root, transform)
    logger.info(
        f"Loaded SYN dataset: train={len(syn_train)}, test={len(syn_test)}")
    ...
    combined_sampled_test = Subset(combined_test,
                                   combined_indices[:combined_sample_size])
    logger.info(
    ...
    # 根据样本数量比例分配客户端数量
    total_clients = getattr(config.federate, 'client_num',
                            100)  # 从配置获取总客户端数，默认100
    client_allocation = calculate_client_allocation(dataset_train_sizes,
                                                    total_clients)

    logger.info("Client allocation based on dataset sizes:")
    for i, (name, size, clients) in enumerate(
            zip(dataset_names, dataset_train_sizes, client_allocation)):
        percentage = (size / sum(dataset_train_sizes)) * 100
        logger.info(
            f"{name}: {size} samples ({percentage:.1f}%) -> {clients} clients")
    ...
    logger.info(
        f"Successfully created federated MNIST dataset with {len(mnist_data_dict) - 1} clients"
    )
    ...
    logger.info(
        f"Successfully created federated USPS dataset with {len(usps_data_dict) - 1} clients"
    )
    ...
    logger.info(
        f"Successfully created federated SVHN dataset with {len(svhn_data_dict) - 1} clients"
    )
    ...
    logger.info(
        f"Successfully created federated SYN dataset with {len(syn_data_dict) - 1} clients"
    )

    data_dicts = [
        mnist_data_dict, usps_data_dict, svhn_data_dict, syn_data_dict
    ]
