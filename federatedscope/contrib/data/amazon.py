...
def load_amazon_dataset(config, client_cfgs=None):
    """
    Load Amazon dataset for federated learning
    ...
    try:
        ...
        logger.info(f"Loaded Amazon dataset: {len(train_dataset)} train samples, {len(test_dataset)} test samples")
...
def call_amazon_data(config, client_cfgs=None):
    """
    Entry point for Amazon dataset registration.
    ...
    """
    if config.data.type.lower() == "amazon":
        logger.info("Loading Amazon dataset...")
        return load_amazon_dataset(config, client_cfgs)
    return None


# Register the Amazon dataset
register_data("amazon", call_amazon_data)