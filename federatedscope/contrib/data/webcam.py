...
def load_webcam_dataset(config, client_cfgs=None):
    """
    Load Webcam dataset for federated learning
    ...
    # Define transforms based on RethinkFL implementation
    ...
    try:
        ...
        logger.info(f"Loaded Webcam dataset: {len(train_dataset)} train samples, {len(test_dataset)} test samples")

    except Exception as e:
        logger.error(f"Failed to load Webcam dataset: {e}")
        raise
...
def call_webcam_data(config, client_cfgs=None):
    """
    Entry point for Webcam dataset registration.
    ...
    """
    if config.data.type.lower() == "webcam":
        logger.info("Loading Webcam dataset...")
        return load_webcam_dataset(config, client_cfgs)
    return None


# Register the Webcam dataset
register_data("webcam", call_webcam_data)