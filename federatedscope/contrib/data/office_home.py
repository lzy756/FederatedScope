...
class OfficeFullImages:
    ...
    def __init__(self, domain_name, root):
        ...
        # Build the path to the Office Home dataset
        dataset_path = os.path.join(self.root, "OfficeHomeDataset", self.domain_name)
...
def load_office_home_dataset(config, client_cfgs=None):
    """
    Load Office Home dataset for federated learning.
...
def call_office_home_data(config, client_cfgs=None):
    """
    ...
    """
    if config.data.type.lower() == "office_home":
        logger.info("Loading Office Home dataset...")
        return load_office_home_dataset(config, client_cfgs)
    return None


# Register the Office Home dataset
register_data("office_home", call_office_home_data)
