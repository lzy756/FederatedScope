...
class RESISC45Splitter(BaseSplitter):
    ...
    def __call__(self, dataset, *args, **kwargs):
        ...
        if hasattr(dataset, 'targets'):
            ...
            labels = dataset.targets.tolist() if hasattr(dataset.targets, 'tolist') else list(dataset.targets)
        elif hasattr(dataset, 'labels'):
            # 对于某些数据集，标签存储在labels属性中
            labels = dataset.labels.tolist() if hasattr(dataset.labels, 'tolist') else list(dataset.labels)
        else:
            # 回退到逐个访问（较慢）
            logger.info(f"Using slow label extraction for dataset of length {len(dataset)}")
        ...
        for client_idx, selected_classes in enumerate(
                client_selected_classes_list):
            ...
            client_sample_indices[client_idx].extend(
                list(current_client_indices_set))

        # 将索引列表转换为 Subset 对象列表
        client_datasets = [
            Subset(dataset, idxs) for idxs in client_sample_indices
        ]
