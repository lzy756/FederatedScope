...
class AIDSplitter(BaseSplitter):
    def __init__(self, client_num, **kwargs):
        super(AIDSplitter, self).__init__(client_num, **kwargs)

    def __call__(self, dataset, *args, **kwargs):
        """
        为每个客户端分配随机的5个类别数据。允许数据在客户端之间重复。
...
def call_my_splitter(splitter_type, client_num, **kwargs):
    if splitter_type == 'AIDsplitter':
        splitter = AIDSplitter(client_num, **kwargs)
        return splitter


register_splitter('AIDsplitter', call_my_splitter)
