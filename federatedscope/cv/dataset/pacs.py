"""
PACS Dataset for Domain Adaptation in Federated Learning
...
Reference:
    Li, D., Yang, Y., Song, Y. Z., & Hospedales, T. M. (2017).
...
class PACS(Dataset):
    ...
    Args:
        ...
        domain: which domain to load ('photo', 'art_painting', 'cartoon', 'sketch')
    ...
    # 7 common classes across all domains
    CLASSES = [
    ...
    def _load_data(self):
        ...
        # PACS dataset structure: root/pacs_data/domain/class/image.jpg
        pacs_data_dir = osp.join(self.root, 'pacs_data')
        domain_dir = osp.join(pacs_data_dir, self.domain)

        if not osp.exists(domain_dir):
            raise FileNotFoundError(
                f"Domain directory not found: {domain_dir}\n"
                f"Please check that PACS dataset is correctly placed.\n"
                f"Expected structure: {self.root}/pacs_data/{self.domain}/class_name/images"
            )
...
def load_pacs_domain_data(root,
                          domain,
                          splits=(0.8, 0.1, 0.1),
                          transform=None,
                          val_transform=None,
                          ...
                          exclude_indices=None):
    """
    Load PACS dataset for a single domain
...
def load_balanced_pacs_data(root,
    ...
    Returns:
        ...
        - 'excluded_indices': dict mapping domain names to sets of excluded indices
                             (to be passed to PACS when loading client data)
    ...
    for domain in PACS.DOMAINS:
        try:
            # PACS dataset structure: root/pacs_data/domain/class/image.jpg
            pacs_data_dir = osp.join(root, 'pacs_data')
            domain_dir = osp.join(pacs_data_dir, domain)
            ...
            for class_idx, class_name in enumerate(PACS.CLASSES):
                class_dir = osp.join(domain_dir, class_name)
            ...
            for class_idx in range(len(PACS.CLASSES)):
                ...
                if samples_per_class is not None:
                    ...
                    if len(class_image_paths) < samples_per_class:
                        logger.warning(f"Domain {domain}, class {PACS.CLASSES[class_idx]}: "
                    ...
                    logger.debug(f"Domain {domain}, class {PACS.CLASSES[class_idx]}: "
