...
from federatedscope.core.communication import (
    StandaloneCommManager,
    StandaloneDDPCommManager,
    gRPCCommManager,
)
...
class Client(BaseClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(config.attack.attacker_id, int):
            ...
            raise TypeError(
                f"The expected types of config.attack.attack_id "
        ...
        self.is_attacker = (
            ID in parsed_attack_ids
            and config.attack.attack_method != ""
            and config.federate.mode == "standalone"
        )
        ...
        patience = (
            self._cfg.early_stop.patience
            if self._cfg.federate.method in ["local", "global"]
            else 0
        )
        self.early_stopper = EarlyStopper(
            patience,
            self._cfg.early_stop.delta,
        ...
        if "resource_info" in kwargs and kwargs["resource_info"] is not None:
            self.comp_speed = (
                float(kwargs["resource_info"]["computation"]) / 1000.0
            )  # (s/sample)
            self.comm_bandwidth = float(
                kwargs["resource_info"]["communication"]
            )  # (kbit/s)
        else:
        ...
        if self.mode == "standalone":
            ...
            self.comm_manager.add_neighbors(
                ...
                address={"host": server_host, "port": server_port},
            )
            self.local_address = {
        ...
        for model_index in range(len(init_model)):
            ...
            for key in init_model[model_index].keys():
                if key not in updated_model[model_index].keys():
                    continue
                model_delta[key] = (
                    updated_model[model_index][key] - init_model[model_index][key]
                )
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        if "ss" in message.msg_type:
                ...
                self.comm_manager.send(
                    Message(
                        ...
                        content=(
                            sample_size,
                            (
                                first_aggregate_model_para[0]
                                if single_model_case
                                else first_aggregate_model_para
                            ),
                        ),
                    )
                )
            ...
            skip_train_isolated_or_global_mode = (
                self.early_stopper.early_stopped
                and self._cfg.federate.method in ["local", "global"]
            )
            if self.is_unseen_client or skip_train_isolated_or_global_mode:
                ...
                if (
                    self.early_stopper.early_stopped
                    and self._monitor.local_convergence_round == 0
                ):
                    logger.info(
                ...
                if (
                    ...
                    and not self._cfg.federate.online_aggr
                ):
                ...
                if self._cfg.wandb.use and self._cfg.wandb.client_train_info:
                    self._monitor.save_formatted_results(
                        train_log_res, save_file_name=""
                    )
            ...
            if self._cfg.federate.use_ss:
                ...
                for neighbor in self.comm_manager.neighbors:
                    if neighbor != self.server_id:
                        content_frame = (
                            model_para_list_all[0][frame_idx]
                            if single_model_case
                            else [
                                model_para_list[frame_idx]
                                for model_para_list in model_para_list_all
                            ]
                        )
                        self.comm_manager.send(
                            Message(
                                msg_type="ss_model_para",
                                sender=self.ID,
                                receiver=[neighbor],
                                state=self.state,
                ...
                self.comm_manager.send(
                    Message(
                        ...
                        state=self.state,
                        timestamp=self._gen_timestamp(
                            init_timestamp=timestamp, instance_number=sample_size
                        ),
        ...
        for requirement in requirements:
            if requirement.lower() == "num_sample":
                if self._cfg.train.batch_or_epoch == "batch":
                    num_sample = (
                        self._cfg.train.local_update_steps
                        * self._cfg.dataloader.batch_size
                    )
                else:
                    num_sample = self._cfg.train.local_update_steps * len(
                        self.trainer.data.train_data
                    )
                ...
                assert (
                    self.comm_bandwidth is not None and self.comp_speed is not None
                ), ("The requirement join_in_info " "'client_resource' does not exist.")
                join_in_info["client_resource"] = (
                    self.model_size / self.comm_bandwidth + self.comp_speed
                )
            else:
                raise ValueError(
    ...
    def callback_funcs_for_evaluate(self, message: Message):
        ...
        if message.content is not None:
            # logger.info(
            #     f"Client #{self.ID} received model parameters from {message.sender} : {message.content.keys()}"
            # )
            self.trainer.update(
