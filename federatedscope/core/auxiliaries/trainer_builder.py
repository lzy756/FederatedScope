...
def get_trainer(
    ...
    if config.trainer.type == "general":
        ...
        trainer_cls = getattr(
            importlib.import_module(name=dict_path),
        ...
        from federatedscope.vertical_fl.tree_based_models.trainer.utils import (
            get_vertical_trainer,
        )

        trainer = get_vertical_trainer(
            config=config, model=model, data=data, device=device, monitor=monitor
        )
    else:
        ...
        if trainer is None:
            raise ValueError("Trainer {} is not provided".format(config.trainer.type))

    if not isinstance(trainer, Trainer):
        logger.warning(
            f"Hook-like plug-in functions cannot be enabled when "
    ...
    if config.federate.method.lower() == "pfedme":
        ...
        trainer = FedEMTrainer(
            model_nums=config.model.model_num_per_trainer, base_trainer=trainer
        )
    ...
    if is_attacker:
        ...
        logger.info("----- This client is a benign client for backdoor attacks -----")
