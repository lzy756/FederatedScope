...
def extend_data_cfg(cfg):
    ...
    # cross-domain learning related options
    cfg.data.clients_per_domain = []
    cfg.data.classes_per_client_per_domain = []
...
def assert_data_cfg(cfg):
    if cfg.dataloader.type == "graphsaint-rw":
        assert cfg.model.layer == cfg.dataloader.walk_length, (
            "Sample " "size " "mismatch"
        )
    if cfg.dataloader.type == "neighbor":
        assert cfg.model.layer == len(cfg.dataloader.sizes), "Sample size mismatch"
    if "@" in cfg.data.type:
        assert cfg.federate.client_num > 0, (
            "`federate.client_num` should "
            "be greater than 0 when using "
            "external data"
        )
        assert cfg.data.splitter, (
            "`data.splitter` should not be empty when " "using external data"
        )
