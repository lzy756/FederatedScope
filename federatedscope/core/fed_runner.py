...
class BaseRunner(object):
    ...
    def __init__(
        ...
        self.gpu_manager = GPUManager(
            gpu_available=self.cfg.use_gpu, specified_device=self.cfg.device
        )
        ...
        if self.cfg.federate.unseen_clients_rate > 0:
            self.unseen_clients_id = np.random.choice(
                ...
                size=max(
                    1,
                    int(
                        self.cfg.federate.unseen_clients_rate
        ...
        self.resource_info = get_resource_info(config.federate.resource_info_file)
    ...
    def _setup_server(self, resource_info=None, client_resource_info=None):
        ...
        Returns:
            Instantiate server.
        """
        assert self.server_class is not None, "`server_class` cannot be None."
        self.server_id = 0
        server_data, model, kw = self._get_server_args(
            resource_info, client_resource_info
        )
    ...
    def _setup_client(self, client_id=-1, client_model=None, resource_info=None):
        """
        ...
        Returns:
            Instantiate client.
        """
        assert self.client_class is not None, "`client_class` cannot be None"
        self.server_id = 0
        ...
        client_device = (
            ...
            else self.gpu_manager.auto_choice()
        )
        ...
        if client_id == -1:
            logger.info(
                "Client (address {}:{}) has been set up ... ".format(
    ...
    def check(self):
        ...
        try:
            ...
            for flag, msg_handler_dict in zip(flags.keys(), msg_handler_dicts):
                ...
                for msg_in, (handler, msgs_out) in msg_handler_dict.items():
            ...
            if nx.has_path(G, "Client_join_in", "Server_finish"):
                if nx.is_weakly_connected(G):
                    logger.info(
                        f"Completeness check passes! Save check "
...
class StandaloneRunner(BaseRunner):
    def _set_up(self):
        ...
        logger.info(f"data: {type(self.data)}, {len(self.data)}")
        ...
        if self.resource_info is not None:
            ...
            client_resource_info = [self.resource_info[x] for x in sampled_index[1:]]
        else:
        ...
        self._shared_client_model = (
            get_model(self.cfg.model, self.data[1], backend=self.cfg.backend)
            if self.cfg.federate.share_local_model
            else None
        )
        for client_id in range(1, self.cfg.federate.client_num + 1):
            self.client[client_id] = self._setup_client(
                ...
                resource_info=(
                    ...
                    if client_resource_info is not None
                    else None
                ),
            )
        ...
        while True:
            if len(self.shared_comm_queue) > 0:
                ...
                if self.cfg.asyn.use and self.cfg.asyn.aggregator == "time_up":
                    ...
                    if len(self.shared_comm_queue) == 0 and len(server_msg_cache) == 0:
                        break
                else:
...
class FedRunner(object):
    ...
    def __init__(
        ...
        logger.warning(
            "`federate.core.fed_runner.FedRunner` will be "
    ...
    def _setup_server(self, resource_info=None, client_resource_info=None):
        ...
        if self.mode == "standalone":
            ...
            raise ValueError("Mode {} is not provided".format(self.cfg.mode.type))
    ...
    def _setup_client(self, client_id=-1, client_model=None, resource_info=None):
        ...
        if self.client_class:
            client_specific_config = self.cfg.clone()
            if self.client_cfgs and self.client_cfgs.get("client_{}".format(client_id)):
                client_specific_config.defrost()
                client_specific_config.merge_from_other_cfg(
                    self.client_cfgs.get("client_{}".format(client_id))
                )
