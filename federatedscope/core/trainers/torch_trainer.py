...
class GeneralTorchTrainer(Trainer):
    ...
    def setup_data(self, ctx):
        ...
        if isinstance(ctx.data, ClientData):
            ...
            logger.warning(
                f"The data type should be `ClientData` to "
            ...
            for split in data.keys():
                ...
                if data.get(split, None) is not None:
                    if isinstance(data.get(split), Dataset):
                        ...
                        raise TypeError(
                            "Type {} is not supported.".format(type(data.get(split)))
                        )
        else:
    ...
    def update(self, model_parameters, strict=False):
        ...
        merged_param = merge_param_dict(
            self.ctx.model.state_dict().copy(), self._param_filter(model_parameters)
        )
    ...
    def _hook_on_data_parallel_init(self, ctx):
        ...
        if len(ctx.cfg.train.data_para_dids):
            ctx.model = torch.nn.DataParallel(
                ctx.model, device_ids=ctx.cfg.train.data_para_dids
            )
    ...
    def _hook_on_batch_start_init(self, ctx):
        ...
        try:
            ctx.data_batch = CtxVar(
                next(ctx.get("{}_loader".format(ctx.cur_split))), LIFECYCLE.BATCH
            )
    ...
    def _hook_on_batch_end(self, ctx):
        ...
        ctx.loss_regular_total += float(ctx.get("loss_regular", 0.0))
